...
export default function CourseTypeSalesView({
  ...
  const handleDeleteCustomerCourse = async (
    ...
    try {
      ...
      await Promise.all([
        trpcUtils.courseSales.typeSummary.invalidate(),
        trpcUtils.courseSales.typeCustomers.invalidate(),
      ]);
  ...
  const startCourseChange = (entry: any) => {
    const initialCourseId = entry.courseId || courseEditorCustomer?.profileCourseId || '';
